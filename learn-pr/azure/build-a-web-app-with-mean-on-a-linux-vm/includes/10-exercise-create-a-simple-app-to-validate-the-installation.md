I den här enheten, ska du skapa en enkel AngularJS-program i Node.js och använda Express för routning. I serverdelen fungerar MongoDB som datalager. Programmet är en bokdatabas där du kommer att kunna visa, lägga till och ta bort böcker.

> [!Important]
> Det här är ett enkelt program. Syftet är att testa nyinstallerade MEAN-stacken. Det här programmet är inte tillräckligt säkert eller redo för produktionsanvändning.

## <a name="create-the-application"></a>Skapa programmet

Först ska vi skapa kod, skript och HTML-filer för vårt program. Vi ska göra detta i Cloud Shell-redigeraren och sedan kopiera filerna till den virtuella datorn.

1. I Cloud Shell, om du är fortfarande ansluten med SSH till den virtuella datorn använder `exit` att återgå till Cloud Shell-filsystem.
...
1. Skapa datamodellen Mongoose. I redigeraren, öppna `app/model.js` och klistra in följande kod.
    ...
    > [!IMPORTANT]
    > När du klistra in koden i en fil i redigeraren, se till att spara efteråt med `Ctrl+S`.
...
2. Skapa Express-vägar som hanterar HTTP-begäranden. Öppna `app/routes.js` i redigeraren och klistra in följande kod.
...
3. Skapa servern Express för att vara värd för programmet. Öppna `server.js` i redigeraren och klistra in följande kod:
...
4. Skapa JavaScript-program på klientsidan. Öppna `public/script.js` i redigeraren och klistra in den här koden:
...
6. Vi är klara redigering filer. Kontrollera att du har sparat dem kör sedan följande kommando för att kopiera dem till den virtuella datorn. Ange ditt lösenord när du tillfrågas.

    ```bash
    scp -r ~/Books <vm-admin-username>@<vm-public-ip>:~/Books
    ```

## <a name="install-node-packages"></a>Installera paket i noden
...
1. Ändra sökvägen till den `Books` directory.
...
1. Installera **body-parsern** för att förbearbeta JSON-begärandedata för användning i vår Express-routning.
    ...
    > [!TIP]
    > När du installerar flera npm-paket, kan du inkludera dem allt i ett enda kommando som detta:
    >
    > ```bash
