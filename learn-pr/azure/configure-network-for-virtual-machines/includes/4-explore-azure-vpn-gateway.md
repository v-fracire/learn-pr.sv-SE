Om du vill integrera din lokala miljö med Azure behöver du ha möjlighet att skapa en krypterad anslutning. Du kan ansluta via det offentliga internet eller via en dedikerad länk. Här ska vi titta på Azures VPN-gateway, vilket ger en slutpunkt för inkommande anslutningar från lokala miljöer.
...
- VNET till VNET-anslutningar via IPsec/IKE VPN-tunnel, länkning av VPN-gatewayer till andra VPN-gatewayer.
...
När du planerar en VPN-gateway finns det tre arkitekturer som vi behöver tänka på:
...
| Vanliga bandbredder         | Beror på gateway-SKU    | Upp till 1 Gbit/s med aggregering         | Från 50 Mbit/s till 10 Gbit/s       |
...
| SKU              |  S2S/VNet-till-VNet-tunnlar | P2S-anslutningar  |  Prestandamått för aggregerat dataflöde   | Använd för                         |
| -------------    | -------------             | -------------    | ---------                         | ---------                       |
...
> [!Note] 
> Det är viktigt att du väljer rätt SKU. Om du har konfigurerat din VPN-gateway med fel SKU måste du ta bort det och återskapa gatewayen, vilket kan vara tidskrävande
...
- Undernät får inte överlappa varandra   Det är mycket viktigt att ett undernät på en plats inte innehåller samma adressutrymme som på en annan plats.

- IP-adresser måste vara unika   Du kan inte ha två värdar med samma IP-adress på olika platser, eftersom det blir omöjligt att dirigera trafik mellan dessa två värdar och VNET till VNET-anslutningen kan inte upprättas.

- VPN-gatewayer behöver ett gatewayundernät som kallas **GatewaySubnet** Det måste ha det här namnet för att gatewayen ska fungera och det får inte innehålla några andra resurser.

### <a name="create-an-azure-virtual-network"></a>Skapa ett virtuellt Azure-nätverk

Innan du skapar en VPN-gateway behöver du skapa det virtuella Azure-nätverket.
...
En Azure VPN-gateway är en komponent i virtuella Azure-nätverk som gör det möjligt att aktivera punkt till plats-, plats till plats- eller vnet till vnet-anslutningar. Med Azure VPN-gatewayer är det möjligt att aktivera enskilda klientdatorer för att ansluta till resurser i Azure, utöka lokala nätverk till Azure eller underlätta anslutningar mellan virtuella nätverk i olika regioner och prenumerationer.