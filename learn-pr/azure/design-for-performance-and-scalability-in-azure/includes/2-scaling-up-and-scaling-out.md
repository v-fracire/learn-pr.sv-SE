...
I en värld där programefterfrågan är konstant är det enkelt att förutse hur mycket resurser du behöver. I verkligheten varierar kraven på programmen, och då blir det svårare att förutse vad som kommer behövas. Om du har tur kan förändringarna vara förutsebara eller säsongsberoende, men det gäller inte alla program. Det mest optimala är att etablera rätt mängd resurser för att möta efterfrågan och göra justeringar då efterfrågan ändras.
...
#### <a name="scaling-up-and-out"></a>Skala upp och ut
...
- Azure SQL Database är en plattform som en tjänst-implementering (PaaS) av Microsoft SQL Server.  Du kan skala upp en databas baserat på antalet databastransaktionsenheter (DTU) eller virtuella processorer. DTU:er är en abstraktion av underliggande resurser och är en blandning av CPU, IO och minne. Du kan exempelvis skala din Azure SQL-databas från en storlek på P2 med 250 DTU:er upp till en P4 med 500 DTU:er för att ge databasen mer dataflöde och kapacitet.
...
Automatisk skalning ger även den extra fördelen att ha beredskap inför oförutsedda scenarier. Webbplatsen kan plötsligt få högre belastning under en helg (fler tidsbokningar under vintersäsongen på grund av förkylningar och influensa). Teamet kan ställa in automatisk skalning för att öka med en instans när CPU-användningen ligger över 70 % och minska med en instans när användningen går under 15 %.
