...
En nyligen inträffad dataläcka från skadliga aktörer har tvingad företaget att stärka sina lösenordsprinciper. De kräver nu att användare ändrar sina lösenord oftare och att de använder längre, mer avancerade lösenord. Detta har medfört den oönskade följden att användare antecknar komplexa lösenord på ett osäkert sätt eftersom de har svårt att komma ihåg flera uppsättningar med autentiseringsuppgifter som har skapats för olika administrativa roller. 

Här kan vi kommer att diskutera identitet som ett extra säkerhetslager för interna och externa program, fördelarna med enkel inloggning (SSO) och multifaktorautentisering (MFA) att tillhandahålla identitetssäkerhet och varför ska du överväga att replikera lokala identiteter till Azure Active Directory.
...
Azure AD Connect integrerar dina lokala kataloger med Azure Active Directory. Azure AD Connect ger de nyaste funktionerna och ersätter äldre versioner av identitetsintegrationsverktyg som DirSync och Azure AD Sync.

Det är ett enda verktyg för att ange en enkel distributionsupplevelse för synkronisering och logga in.
...
- Använda Azure AD Connect för att synkronisera grupper, användarkonton och lösenords-hash som lagras i sina lokala Active Directory till Azure AD
  - Detta kan användas som reservmetod om direktautentisering är inte tillgänglig
...
Lamna Healthcares säkerhetsprincip kräver att alla inloggningar som sker utanför företagets perimeternätverk autentiseras med ytterligare en autentiseringsfaktor. Det här kravet kombinerar två aspekter av Azure AD-tjänsten: Multi-Factor authentication och principer för villkorlig åtkomst.
...
Tillsammans med multifaktorautentisering går det att lägga till ytterligare en skyddsnivå genom att se till att ytterligare krav är uppfyllda innan åtkomst beviljas. Blockera inloggningar från misstänkta IP-adress eller neka åtkomst från enheter utan skydd mot skadlig kod kan begränsa åtkomst från riskfyllda inloggningar.

Azure Active Directory tillhandahåller en funktion för principer för villkorsstyrd åtkomst (CAP) som har stöd för åtkomstprinciper baserade på grupp, plats eller enhetstillstånd. Plats-funktionen tillåter Lamna att skilja IP-adresser som inte tillhör sina nätverk och uppfyller deras säkerhetsprincip för att kräva multifaktorautentisering från alla platser.
...
Lamna Healthcares anställda behöver säker fjärråtkomst till sina administrativa program som finns lokalt. Användare autentiserar för närvarande till programmet med hjälp av integrerad Windows-autentisering (WIA) från sina domänanslutna datorer bakom företagets brandvägg. Ett projekt att införliva moderna autentiseringsmekanismer i programmet har planerats, finns men det betydande företag tryck för att aktivera fjärråtkomst-funktionerna så snart som möjligt. Azure Application Proxy kan snabbt, enkelt och säkert kan programmet nås via en fjärranslutning utan några kodändringar.
...
- Kostnadseffektivt
  - Programproxy fungerar i molnet så att du kan spara tid och pengar. Lokala lösningar kräver normalt att du konfigurerar och underhåller DMZ:er, edge-servrar eller andra komplexa infrastrukturer.

Azure AD Application Proxy består av två komponenter: en connector-agent som finns på en Windows-server i företagsnätverket och en extern slutpunkt MyApps-portalen eller en extern URL. När en användare navigerar till slutpunkten autentiserar denne med Azure AD och dirigeras till det lokala programmet via anslutningsagenten.

## <a name="working-with-consumer-identities"></a>Arbeta med konsumentidentiteter

Sedan integrera modern autentisering med sina befintliga program kan har Lamna Healthcare snabbt bekräftas kraften en hanterad identitetssystem som Azure AD kan ge sin organisation. Ledningen vill nu utforska andra sätt Microsoft identity-tjänster kan lägga till affärsvärde. De har nu fokuserar deras uppmärksamhet på externa kunder och hur modernisering av befintliga kundinteraktioner kunde ger tät integrering med tredje parts identitetsleverantörer, till exempel Google, Facebook och LinkedIn.
