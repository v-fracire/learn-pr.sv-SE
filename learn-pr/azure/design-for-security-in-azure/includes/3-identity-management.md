...
![En bild som visar hur Azure AD Connect används för att synkronisera dina lokala kataloger med Azure Active Directory och dina appar i Azure-molnet. På så sätt kan användarna ha en gemensam inloggning för alla program i Azure-molnet.](../media/AADCONNECTxprs_960.png)
...
- Använd Azure AD Connect till att synkronisera grupper, användarkonton och lösenordshashar som lagras i deras lokala Active Directory till Azure AD
  - Detta kan användas som en reservmetod om direktautentisering inte är tillgänglig
- Konfigurera direktautentisering med hjälp av en lokal autentiseringsagent som är installerad på en lokal Windows Server
...
Lamna Healthcare har skapat en [princip för villkorsstyrd åtkomst](https://docs.microsoft.com/azure/active-directory/conditional-access/overview) som kräver att användare som har åtkomst till programmet från en IP-adress utanför företagsnätverket prövas med multifaktorautentisering.

![En bild som visar ett exempel på implementering av en villkorlig åtkomstprincip och multifaktorautentisering. Användarförfrågningar om att få åtkomst till lokala och molnbaserade program kontrolleras först mot en villkorslista. Antingen beviljas förfrågningarna åtkomst, tvingas att gå igenom multifaktorautentisering eller blockeras utifrån vilka villkor de uppfyller.](../media/conditional-access.png)
...
- Kostnadseffektivt
  - Programproxy fungerar i molnet så att du kan spara tid och pengar. Lokala lösningar kräver normalt att du konfigurerar och underhåller perimeternätverk, gränsservrar eller andra komplexa infrastrukturer.
