Du har valt att använda en teknik med öppen källkod för att skapa ditt webbprogram. Du vet att ASP.NET Core är ett plattformsoberoende ramverk med öppen källkod. Du bestämmer dig för att utveckla din webbapp i en Linux-utvecklingsmiljö med ASP.NET Core!

Azure App Service kan du använda dina favoritverktyg webbtekniker, inklusive Node.js, PHP eller .NET Core.
...
När du har bestämt dig för att skapa ett webbprogram kan du välja mellan flera språk och ramverk. App Service hjälper till att göra det här alternativet enklare genom att låta dig värd olika typer av program som Node.js, PHP eller .NET Core. På så sätt kan du använda språk och ramverk som du är mest bekväm med i stället för att ändra för att uppfylla kraven för din webbvärd.
