I den här enheten, måste du ladda upp din ASP.NET Core-App till Azure App Service.

## <a name="create-a-staging-deployment-slot"></a>Skapa ett distributionsfack för mellanlagring

1. Öppna den App Service-resurs (webbapp) som du skapade tidigare. Om du har stängt sidan du hittar den igen genom att söka efter appen i **alla resurser** eller den aktuella resursgruppen i **resursgrupper**.

1. Klicka på menykommandot **Distributionsfack** i det vänstra navigeringsfönstret.

1. I den **distributionsfack** klickar du på den **Lägg till plats** knappen i det övre navigeringsfältet på sidan platser för distributionen.

1. Azure-portalen öppnas den **Lägg till en plats** sidan som visas nedan.
    ...
    2. När du väljer **Konfigurationskälla** har du två alternativ.
        ...
        * Du kan välja att klona konfigurationselement från en befintlig distributionsplats eller en App Service-app.
...
1. Klicka på den **OK** längst ned på sidan för att skapa din nya distributionsplats.

1. När distributionsplatsen har skapats, Azure-portalen navigerar du tillbaka till den **distributionsfack** sidan i webbappen.

    Du kan nu se det nya distributionsfacket du just skapade.

    ![Skärmbild av Azure-portalen som visar sidan distributionsplatser med den nya platsen som har skapats.](../media/7-deployment-slot-created.png)
...
1. Azure-portalen navigerar till **Översikt**-sidan för det nyligen skapade distributionsfacket.
    ...
    Ett distributionsfack behandlas som en fullständig App Service-app i Azure. Det är dock en särskild typ som är underordnad den ursprungliga appen och kan därför bytas mot den ursprungliga appen.

    Om du klickar på den **URL**, visas samma standardsidan som Azure skapade för distributionsplatsen ”app” första gången vi skapade den i Azure-portalen.
...
1. Azure-portalen navigerar till den **autentiseringsuppgifter för distribution** sidan som visas nedan.
    ...
    ![Skärmbild av Azure-portalen som visar sidan autentiseringsuppgifter för distribution av mellanlagringsplatsen med exempel autentiseringsuppgifter i de obligatoriska fälten.](../media/7-deployment-credentials.png)

1. Klicka på den **spara** längst upp på den **autentiseringsuppgifter för distribution** sidan.
...
1. I den **mellanlagring** distributionsplats ”app”, klicka på den **distributionsalternativ** menyalternativ i det vänstra navigeringsfönstret.

1. Azure-portalen navigerar till den **distributionsalternativ** sidan.
...
1. Du kommer tillbaka till den **distributionsalternativet** sidan. Klicka på den **OK** längst ned på sidan för att konfigurera någon distributionskälla.
...
1. Navigera nu till avsnittet **Distributionscenter (förhandsversion)** till vänster för att visa den nya distributionsinformationen.
    ...
    ![Skärmbild av Azure-portalen som visar den distributionsplatsen Deployment Center sida med Git klona Uri för facket markerat.](../media/7-staging-after-setting-git.png)

    Viktig information att notera här är den **Uri för Git-klon**, som är lokala Git-lagringsplats-URL som du vill använda som en **remote** för lagringsplatsen för din lokala program.
...
1. Det är alltid en bra idé att konfigurera Git-inställningar genom att ange namn och e-postadress. För att göra det så behöver du utfärda följande kommandon, där du ersätter platshållarna `{your name}` och `{your email}` med ditt eget namn och e-postadress (utan `{}` klammerparanteserna):
...
## <a name="initialize-a-local-git-repository-for-your-code"></a>Initiera en lokal Git-lagringsplats för din kod

Om du vill börja använda Git, måste du initiera en lokal Git-lagringsplats för din programkod i .NET Core.

1. Öppna ett nytt **terminalfönster**.

1. Navigera till rotmappen innehåll för .NET Core-app som du skapade tidigare. Du kan skriva så här:
...
1. Mellanlagra programfiler till Git.

   Nästa steg är att låta Git veta om dina filer. Det gör du genom att lägga till alla filer i arbetskatalogen så att de **mellanlagras** av Git. Ange följande kommando:
...
Hittills har du initierat en ny lokal Git-lagringsplats. Du har dessutom åtagit alla dina filer till Git. Det som återstår är att lägga till en **fjärranslutning** som ansluter din lokala Git-lagringsplats till den som hanteras i Azure.
...
Nu när du har din lokala Git-lagringsplats som kopplas till Git-lagringsplatsen i Azure kommer du utvecklar och skapa appen och skicka sedan din programkod till Azure.
...
## <a name="verify-the-code-is-uploaded-to-azure"></a>Kontrollera koden har laddats upp till Azure
...
1. Klicka på det mellanlagringsfack som skapades ovan. Kom ihåg att en distributionsplats betraktas som en app och därför visas den som en App Service-resurs under **alla resurser**.

1. När du kommer till sidan fack för mellanlagringsplatser distribution går du till **distributionsalternativ**.

    Du kommer att se att din första incheckning som du har lokalt på datorn nu har laddats upp till Azure-portalen.

    När du har överfört din kod lokalt till Git-lagringsplatsen i App Service, registrerar Azure den här åtgärden.
    ...
    ![Skärmbild av Azure-portalen som visar en nyligen genomförd distribution för Git-lagringsplats på alternativsidan för utveckling.](../media/7-staging-deployment-slot-after-uploading-files.png)
...
Du har laddats upp dina lokala filer till en mellanlagringsplats för distribution på Azure.
...
1. Gå till sidan för ursprungliga skapade tidigare. Du kan hitta den ursprungliga webbappen från den **alla resurser** sidan.

1. Klicka på menykommandot **Distributionsfack** i det vänstra navigeringsfönstret.

1. Klicka på den **växla** längst upp på sidan.

1. Azure-portalen navigerar du till den **växla** sidan.
...
1. I fältet **Mål** väljer du **Produktion**.
    ...
    ![Skärmbild av Azure-portalen som visar sidan deployment slot swap.](../media/7-swap-blade.png)

1. Klicka på den **OK** längst ned på sidan.
...
1. Besök nu URL:en för mellanlagringsfacket: [https://bestbike-staging.azurewebsites.net/](https://bestbike-staging.azurewebsites.net/).
    ...
    Mellanlagringsplatsen distributionen fungerar nu ursprungliga standard HTML-filer som tidigare har hämtats från produktionsplatsen.

Grattis! Du har överfört din programkod till Azure och växlas distributionsplatser.
