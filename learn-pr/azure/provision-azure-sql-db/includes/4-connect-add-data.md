...
Vi står för infrastrukturen, uppdateringar och korrigeringar till din Azure SQL Database. Utöver det kan du hantera din Azure SQL Database som vilken SQL Server-installation som helst. Du kan till exempel använda Visual Studio, SQL Server Management Studio eller andra verktyg för att hantera din Azure SQL Database.
...
* Med `sqlcmd` kan du köra instruktioner på SQL Server. Du kommer att använda `sqlcmd` för att skapa en interaktiv session med din Azure SQL Database.

## <a name="get-information-about-your-azure-sql-database"></a>Hämta information om Azure SQL Database
...
CRUD handlar om att **skapa**, **läsa**, **uppdatera** och **ta bort**. De här åtgärderna kan användas tillsammans med tabelldata och utgör de fyra grundläggande funktionerna som du behöver för din app. Nu är det dags att kontrollera att du kan utföra var och en av dem.
...
1. Kör T-SQL-uttrycket `INSERT` för att lägga till en exempelrad i tabellen. Det här är åtgärden **create** (skapa).
    ...
    (1 rows affected)
    ```
1. Kör T-SQL-uttrycket `SELECT` för att visa en lista med `DriverID`-kolumner från alla rader i tabellen. Det här är åtgärden **read** (läsa).
    ```sql
...
Nu har du fått lite praktisk erfarenhet av att köra kommandon via Azures kommandoradsgränssnitt och visa information om din SQL Azure Database. Som överkurs har du också fått öva på att skriva T-SQL.
